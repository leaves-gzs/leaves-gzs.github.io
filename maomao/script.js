...
// ---------- 资源名称映射 ----------
const resNames = {
    rock: '石头', oll: '石油', ry: '燃油', jyr: '精炼石', rb: '石板',
    zt: '钻头', zg: '钻管', deep: '深度', stj: '采集效率', cjq: '采石机',
    alloy: '合金', circuit: '电路板', part: '零件', core: '核心模块',
    ys1: '1级压缩石板', ys2: '2级压缩石板', ys3: '3级压缩石板', ys4: '4级压缩石板', ys5: '5级压缩石板',
    hfuel: '高能燃料', super_fuel: '超级燃油',
    strong_alloy: '强化合金', smart_chip: '智能芯片', frame: '高级机械框架',
    instrument: '精密仪器', strong_zt: '强化钻头', strong_zg: '强化钻管',
    super_drill: '超级钻具', advanced_furnace: '高级熔炉',
    auto_drill_platform: '自动钻井平台', refinery: '炼油厂',
    quantum_pick: '量子镐', quantum_drill: '量子钻头',
    time_shard: '时空碎片', time_accelerator: '时间加速器',
    energy_core: '能量核心',
    super_cjq: '超级采石机', smart_cjq: '智能采石机', ultimate_cjq: '终极采石机'
};
...
const baseRecipes = [
    ...
    { id: 'b3', name: '石镐', desc: '50石头 → 1石镐 (采集效率变为5)', cost: { rock: 50 }, result: { sg: true, stj: 5 }, unlock: () => !game.sg },
    { id: 'b4', name: '精炼镐', desc: '50精炼石 → 1精炼镐 (采集效率+45)', cost: { jyr: 50 }, result: { jlg: true }, unlock: () => !game.jlg && game.sg },
...
const advancedRecipes = [
    { id: 'r1', name: '强化合金', desc: '2合金 + 1电路板 → 1强化合金', cost: { alloy: 2, circuit: 1 }, result: { strong_alloy: 1 }, unlock: () => game.tech.oil.level >= 5 || game.circuit >= 10 },
    { id: 'r2', name: '智能芯片', desc: '1核心模块 + 2电路板 → 1智能芯片', cost: { core: 1, circuit: 2 }, result: { smart_chip: 1 }, unlock: () => game.core >= 1 && game.step >= 200 },
    { id: 'r3', name: '高级机械框架', desc: '10个1级压缩石板 + 2零件 → 1高级机械框架', cost: { ys1: 10, part: 2 }, result: { frame: 1 }, unlock: () => game.ys1 >= 10 },
    { id: 'r4', name: '精密仪器', desc: '1合金 + 2电路板 + 5燃油 → 1精密仪器', cost: { alloy: 1, circuit: 2, ry: 5 }, result: { instrument: 1 }, unlock: () => game.circuit >= 5 },
    { id: 'r5', name: '高能燃料', desc: '1石油 + 1个1级压缩石板 → 10高能燃料', cost: { oll: 1, ys1: 1 }, result: { hfuel: 10 }, unlock: () => game.ys1 >= 1 },
    { id: 'r6', name: '超级采石机', desc: '1高级机械框架 + 3零件 + 5电路板 → 1超级采石机', cost: { frame: 1, part: 3, circuit: 5 }, result: { super_cjq: 1 }, unlock: () => game.cjq >= 5 },
    { id: 'r7', name: '智能采石机', desc: '1超级采石机 + 1智能芯片 + 10燃油 → 1智能采石机', cost: { super_cjq: 1, smart_chip: 1, ry: 10 }, result: { smart_cjq: 1 }, unlock: () => game.super_cjq >= 1 },
    { id: 'r8', name: '强化钻头', desc: '5钻头 + 1合金 → 1强化钻头', cost: { zt: 5, alloy: 1 }, result: { strong_zt: 1 }, unlock: () => game.zt >= 10 },
    { id: 'r9', name: '强化钻管', desc: '5钻管 + 2石板 → 1强化钻管', cost: { zg: 5, rb: 2 }, result: { strong_zg: 1 }, unlock: () => game.zg >= 10 },
    { id: 'r10', name: '超级钻具', desc: '1强化钻头 + 1强化钻管 + 2合金 → 1超级钻具', cost: { strong_zt: 1, strong_zg: 1, alloy: 2 }, result: { super_drill: 1 }, unlock: () => game.strong_zt >= 1 && game.strong_zg >= 1 },
    { id: 'r11', name: '高级熔炉', desc: '15石板 + 10燃油 → 1高级熔炉', cost: { rb: 15, ry: 10 }, result: { advanced_furnace: true }, unlock: () => game.tech.auto_syn.level >= 3 },
    { id: 'r12', name: '自动钻井平台', desc: '1高级机械框架 + 2智能芯片 + 5燃油 → 1自动钻井平台', cost: { frame: 1, smart_chip: 2, ry: 5 }, result: { auto_drill_platform: true }, unlock: () => game.deep >= 100 },
    { id: 'r13', name: '炼油厂', desc: '10合金 + 5电路板 + 20燃油 → 1炼油厂', cost: { alloy: 10, circuit: 5, ry: 20 }, result: { refinery: true }, unlock: () => game.oll >= 100 },
    { id: 'r14', name: '量子镐', desc: '1合金镐 + 3强化合金 + 1核心模块 + 2个5级压缩石板 → 1量子镐', cost: { alloy_pick: 1, strong_alloy: 3, core: 1, ys5: 2 }, result: { quantum_pick: true }, unlock: () => game.alloy_pick && game.step >= 1000 },
    { id: 'r15', name: '量子钻头', desc: '1激光钻头 + 3智能芯片 + 1能量核心 → 1量子钻头', cost: { laser_drill: 1, smart_chip: 3, energy_core: 1 }, result: { quantum_drill: true }, unlock: () => game.laser_drill && game.deep >= 500 },
    { id: 'r16', name: '时空碎片', desc: '1个5级压缩石板 + 1核心模块 + 5高能燃料 → 1时空碎片', cost: { ys5: 1, core: 1, hfuel: 5 }, result: { time_shard: 1 }, unlock: () => game.ys5 >= 1 },
    { id: 'r17', name: '时间加速器', desc: '1时空碎片 + 50科学点 → 1时间加速器', cost: { time_shard: 1, step: 50 }, result: { time_accelerator: 1 }, unlock: () => game.time_shard >= 1 },
    { id: 'r18', name: '能量核心（高级版）', desc: '1核心模块 + 2个5级压缩石板 + 3强化合金 → 1能量核心', cost: { core: 1, ys5: 2, strong_alloy: 3 }, result: { energy_core: 1 }, unlock: () => game.core >= 1 && game.ys5 >= 2 },
    { id: 'r19', name: '超级燃油', desc: '5高能燃料 + 1智能芯片 → 1超级燃油', cost: { hfuel: 5, smart_chip: 1 }, result: { super_fuel: 1 }, unlock: () => game.smart_chip >= 1 },
    { id: 'r20', name: '终极采石机', desc: '1智能采石机 + 1能量核心 + 2量子镐 → 1终极采石机', cost: { smart_cjq: 1, energy_core: 1, quantum_pick: 2 }, result: { ultimate_cjq: 1 }, unlock: () => game.smart_cjq >= 1 && game.quantum_pick && game.energy_core >= 1 && game.step >= 5000 }
];

// 合并所有配方
const allRecipes = [...baseRecipes, ...advancedRecipes];
...
function compressStone() {
    ...
    let changed = false;
    while (game.rb >= 1000) {
        let cnt = Math.floor(game.rb / 1000);
        game.ys1 += cnt;
        game.rb %= 1000;
        changed = true;
    }
    while (game.ys1 >= 1000) {
        let cnt = Math.floor(game.ys1 / 1000);
        game.ys2 += cnt;
        game.ys1 %= 1000;
        changed = true;
    }
    while (game.ys2 >= 1000) {
        let cnt = Math.floor(game.ys2 / 1000);
        game.ys3 += cnt;
        game.ys2 %= 1000;
        changed = true;
    }
    while (game.ys3 >= 1000) {
        let cnt = Math.floor(game.ys3 / 1000);
        game.ys4 += cnt;
        game.ys3 %= 1000;
        changed = true;
    }
    while (game.ys4 >= 1000) {
        let cnt = Math.floor(game.ys4 / 1000);
        game.ys5 += cnt;
        game.ys4 %= 1000;
        changed = true;
    }
...
function initQuests() {
    game.quests = questTemplates.map(q => ({ ...q, done: false, claimed: false }));
}
...
function autoTick() {
    ...
    if (game.auto_drill_platform) {
        if (game.zt > 0 && game.zg > 0 && Math.floor(Date.now() / 600000) % 1 === 0) {
            game.zt -= 1; game.zg -= 1; game.deep += 1;
        }
    }
    // 炼油厂
    if (game.refinery) {
...
function renderMain() {
    ...
    content.innerHTML = `
        ...
        <div style="margin-top:12px;">
            ...
            <button class="btn btn-success" onclick="startAuto()" ${game.isAutoRunning?'disabled':''}>▶ 开始挂机</button>
...
function renderMine() {
    ...
    content.innerHTML = `
        ...
        <div style="margin:10px 0;">
            <button class="btn btn-primary" onclick="doMine()">⛏️ 采集一次</button>
            <button class="btn btn-warning" onclick="doMine10()">⛏️ 采集10次</button>
        </div>
...
function renderCraft() {
    ...
    baseRecipes.forEach(recipe => {
        ...
        html += `<div class="craft-item" style="${disabled?'opacity:0.6;':''}">
            ...
            <button class="btn btn-primary" onclick="doCraftById('${recipe.id}')" ${disabled?'disabled':''}>合成</button>
        </div>`;
    });
    html += `</div>`;
    // 高级合成
    ...
    advancedRecipes.forEach(recipe => {
        ...
        html += `<div class="craft-item" style="${disabled?'opacity:0.6;':''}">
            ...
            <small>${recipe.desc}</small><br>
            <span style="font-size:0.8rem;color:#aaa;">${reason}</span><br>
            <button class="btn btn-purple" onclick="doCraftById('${recipe.id}')" ${disabled?'disabled':''}>合成</button>
        </div>`;
    });
    // 压缩石板特殊按钮
    html += `<div style="margin-top:16px;">
        <button class="btn btn-warning" onclick="compressStone()">🗜️ 压缩石板 (1000合1)</button>
    </div>`;
    html += `</div>`;
...
function upgradeTech(key) {
    ...
    let cost = tech.st_co * tech.level;
...
function renderDrill() {
    ...
    let html = `<h2>🛠️ 钻井</h2>
        ...
        <div style="margin:10px 0;">
            <label>下钻格数: <input type="number" id="drill-count" value="1" min="1" max="${maxDrill}" style="width:60px;"></label>
            <button class="btn btn-primary" onclick="doDrill()">下钻</button>
        </div>
        ${game.super_drill > 0 ? `<button class="btn btn-warning" onclick="doSuperDrill()">使用超级钻具 (1次下钻5格)</button>` : ''}
    `;
    content.innerHTML = html;
}
...
function loadGame() {
    ...
    try {
        ...
        if (offlineSec > 10) {
            ...
            let gain = Math.floor(rockPerSec * offlineSec * 0.5);
            game.rock += gain;
            game.stats.total_rock += gain;
...
function init() {
    ...
    if (game.rock === 0 && game.oll === 0 && game.ry === 0) {
        game.rock = 100; game.oll = 10; game.ry = 20;
    }
    game.isAutoRunning = false;
